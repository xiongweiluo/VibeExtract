...
> **Extracted:** Mon, 20 Apr 2026 16:19:59 GMT
> **URL:** https://www.pinterest.com/
> **Paradigm:** landing · **Density:** spacious · **Motif:** bold-red-brand, lifestyle-imagery, overlapping-card-composition, pill-buttons, editorial-typography
...
| Brand Accent | #e85c2c | `#e85c2c` |
| Background Page | #ffffff | `#ffffff` |
| Background Surface | #f0eeec | `#f0eeec` |
...
# Layout Analysis: Pinterest Homepage
...
| 2 | **hero** | 8–80% | base (normal flow) | solid-color (`#ffffff`) | imagery + typography |
| 3 | **content** (teaser) | 80–100% | base (normal flow) | solid-color (`#f5f5f5` / light gray) | typography |

### Section Details

**nav (0–8%)**
- Full-width top bar with Pinterest logo (red P + wordmark) flush left
- Three left-side text links: `Explore`, `Shop`
- Four right-center text links: `About`, `Businesses`, `Create`, `News`
- Two CTA buttons far right: `Log in` (red pill), `Sign up` (light gray pill)
- Height: ~68px, position: fixed/sticky

**hero (8–80%)**
- Two-column split layout:
  - **Left column (~55% width)**: Large heading text + pagination dots + two CTA controls
  - **Right column (~45% width)**: Layered/overlapping card imagery composition with floating icon badge
- Animated/rotating headline (currently: "Find ideas for / cozy home decor")
- Pause button (⏸) floated bottom-right of image cluster
- Background: white `#ffffff`

**content teaser (80–100%)**
- Centered large heading: "Bring your favorite ideas to life"
- Background: light gray (~`#f0eeec` or similar warm off-white)
- Truncated — page continues below viewport
...
- **Column count**: 2
- **Left column width**: ~55% of viewport
- **Right column width**: ~45% of viewport
- **Gap**: approximately 32px (xl from spacing scale)
- **Item pattern**: fluid columns, left text-dominant, right image-dominant

### Left Column Internal Layout (Hero)
- **Layout engine**: Flexbox (column)
- **Vertical stacking**:
  1. `h1` heading block (~60px type, 2 lines)
  2. Pagination dot row (5 dots, horizontal, gap ≈ 8px / sm)
  3. CTA row: pill button + inline text link (gap ≈ 24px / lg)
- **Spacing between headline and dots**: ~24px (lg)
- **Spacing between dots and CTAs**: ~40px (estimated, between md and xl)

### Right Column — Image Composition
- **Layout engine**: absolute positioning (layered/stacked cards, not grid)
- **Elements**:
  1. Large background card image (cozy living room, leather sofa) — rotated slightly left
  2. Smaller foreground card image (plaid cushion/sofa) — rotated slightly right, overlapping
  3. Floating orange pin-icon badge (top-right of composition) — `position: absolute`, circular ~64px
  4. Pause button (⏸) — bottom-right, circular ~44px, gray background
- **Masonry note**: Not masonry — deliberately staged overlapping card composition

### Nav Bar — Floating Element
- **Z-index tier**: floating (z > base content)
- **Position type**: `position: fixed` or `sticky`, top: 0
- **Layout engine**: Flexbox (row), `justify-content: space-between`
- **Left cluster**: Logo + 2 nav links
- **Right cluster**: 4 nav links + 2 buttons
- **Button gap**: ~8px (sm) between `Log in` and `Sign up`
...
### Hero Background
- **Value**: `background-color: #ffffff` (solid white)
- **No image background detected** in hero section itself

### Logo / Brand
- **Type**: Inline SVG + text wordmark
- **Composition**: Red circular SVG icon (Pinterest "P" pin mark) + "Pinterest" text in dark/black
- **Color**: Icon = `#e60023` (Pinterest red), Text = `#111111` or near-black

### Card Images (Hero Right Column)
- **Image 1** (large, background): Interior lifestyle photo — cozy living room, leather tan sofa, white round coffee table, potted plant, woven rug
  - Physical Data URL: not explicitly listed → referenced from screenshot only
- **Image 2** (small, foreground): Close-up interior — plaid/tartan cushions on a light sofa/chair
  - Physical Data URL: not explicitly listed
- **Note**: No asset URLs were provided in Physical Data for these images. Marking as **"no asset URL found in Physical Data"** — visually confirmed present.

### Icon Set
- **Floating orange badge**: Inline SVG — pin/save icon (scissors or bookmark variant), white icon on `#e85c2c` orange circle background, ~64px diameter
- **Pause button**: Inline SVG — ⏸ pause icon on `#e8e8e8` gray circle background, ~44px diameter
- **Pinterest logo P**: Inline SVG, red on white
...
|---|---|---|
| `#e60023` | **Brand-Primary** | Pinterest logo icon fill, `Log in` button background, "cozy home decor" heading text, "Join Pinterest for free" button background |
| `#e85c2c` | **Brand-Accent** | Floating pin-save badge background (orange-red circle top-right of image) |
| `#111111` | **Text-Main** | "Find ideas for" heading text (dark near-black), nav link text |
| `#111111` | **Brand-Primary (text)** | Pinterest wordmark next to logo |
| `#ffffff` | **Neutral-Base** | Page background (hero section), nav background, button text on red buttons |
| `#f0eeec` | **Neutral-Surface** | Lower content section background (warm off-white/light gray) |
| `#e8e8e8` | **Neutral-Overlay** | Pause button background, `Sign up` button background (light gray pill) |
| `#767676` | **Text-Muted** | "I already have an account" link text (subdued gray inline link) |
| `#767676` | **Border-Default** | Pagination inactive dots (small circles, inactive state) |
| `#e60023` | **Status-** (active dot) | First pagination dot (active/filled, matches Brand-Primary) |
| `#111111` | **Text-Inverse** | Sign up button text on light background |

### Typography Cross-Reference vs Physical Data

| Element | Visual Estimate | Physical Data | Verdict |
|---|---|---|---|
| Hero heading "Find ideas for" | ~56–64px, bold | `h1: 60px, w:600` | ✅ Match |
| "cozy home decor" line | Same size, heavier weight | `h1: 60px, w:600` | ✅ Match — color differs (red) |
| Nav links | ~14–16px | `body-lg: 16px, w:500` | ✅ Match |
| CTA button text | ~14–16px | `body-lg: 16px, w:500` | ✅ Match |
| Content teaser heading | ~48–60px | `h3: 36px OR h1: 60px` | Likely `h1 60px` given visual scale |

**Key Discrepancy**: Physical Data lists `h3` at 36px and `h1` at 60px. The hero heading visually renders at a very large scale consistent with **60px** — no 36px headings appear to be in use in the visible viewport. The base body is declared as **12px** (1rem), which is smaller than typical web defaults (16px); all type appears scaled proportionally from this smaller base.
