...
> **Extracted:** Tue, 21 Apr 2026 09:28:40 GMT
> **URL:** https://linear.app/
> **Paradigm:** saas-app · **Density:** comfortable · **Motif:** deep-black minimal, typographic hero, dark-panel product UI, monochromatic surfaces, precision-engineered
...
| Brand Secondary | #1e2024 | `#1e2024` |
| Brand Accent | #5e6ad2 | `#5e6ad2` |
| Background Page | #0a0a0a | `#0a0a0a` |
| Background Surface | #161618 | `#161618` |
...
| Border | #2a2d35 | `#2a2d35` |
| Status Success | #4caf7d | `#4caf7d` |
...
### 1. Navigation Bar
- **Name**: nav
- **Vertical span**: 0–8% (≈ 0–72px)
- **Z-layer**: floating (fixed/sticky)
- **Background**: solid-color — near-black `#0a0a0a` / `#111111`
- **Dominant visual**: interactive-controls (nav links + CTA buttons)
- **Elements**: Logo (icon + "Linear" wordmark), nav links (Product, Resources, Customers, Pricing, Now, Contact), divider, "Log in" text link, "Sign up" filled button

### 2. Hero Section
- **Name**: hero
- **Vertical span**: 8–58% (≈ 72–520px)
- **Z-layer**: base
- **Background**: solid-color — deep black/near-black `#0a0a0a`, upper portion has subtle dark radial/atmospheric glow
- **Dominant visual**: typography
- **Elements**: Large headline (2-line), subheadline (2-line body text), product UI screenshot/mockup below fold

### 3. Product UI Demo / App Screenshot
- **Name**: content (product demo)
- **Vertical span**: 58–100% (≈ 520–900px, partially cut off)
- **Z-layer**: base
- **Background**: dark panel UI embedded within hero — slightly lighter dark surface `#1a1a1a`/`#161616`
- **Dominant visual**: imagery (app screenshot/interactive mockup)
- **Elements**: Left sidebar (Inbox, My Issues, Reviews, Pulse, Workspace nav), center issue detail panel ("Faster app launch"), right metadata panel (status, priority, assignee, cycle)
...
- **Column count**: 2 groups — left (logo), right (nav links + auth buttons)
- **Gap size**: nav links spaced ~28–32px apart horizontally
- **Item pattern**: fluid (space-between)
- **Note**: Divider (`|`) separator between nav links and Log in/Sign up

### Hero Typography Block
- **Layout engine**: block/flow (single column)
- **Column count**: 1
- **Gap size**: heading-to-subtext gap ≈ 32–40px
- **Item pattern**: fixed-width left-aligned column, max-width ~860px

### Product Demo UI (App Mockup)
- **Layout engine**: CSS Grid or Flexbox — 3 column panel layout
- **Column count**: 3 (sidebar ~220px | main content ~660px | right metadata panel ~250px)
- **Gap size**: 0 (panel borders/dividers, no gap)
- **Item pattern**: fixed-width columns, fluid center
- **Masonry note**: None — flat panel layout

### Floating Elements
- **Nav**: position=fixed or sticky, z-index tier likely z=100+
- No modals, drawers, or cookie banners detected in viewport
...
### Hero bg
- **Value**: Dark solid background with subtle radial gradient glow — no image URL detected from Physical Data. Likely CSS `background: radial-gradient(...)` or `background-color: #000` with layered pseudo-elements. **No asset URL confirmed.**

### Logo / Brand
- **Type**: Composite — small geometric diamond/hexagon SVG icon + "Linear" text wordmark
- **Color**: White `#ffffff`
- **Source**: inline-svg (icon) + text, or SVG sprite — **no external img URL confirmed from Physical Data**

### Card Images
- **No card grid present** in visible viewport. Product UI mockup appears to be either:
  - A rendered `<img>` asset (screenshot of the Linear app), or
  - A live embedded iframe/component
- **Physical Data assets**: No explicit card image URLs provided — **no asset found**

### Icon Set
- **Nav icons**: None visible in nav links
- **Sidebar icons**: Small inline SVG icons next to each sidebar item (Inbox, My Issues, Reviews, Pulse, Initiatives, Projects)
- **Right panel icons**: Status circle, priority bars, avatar circles — inline SVG
- **Pattern**: inline-svg set, monochromatic, muted gray tones
...
|---|---|---|
| `#000000` / `#0a0a0a` | **Neutral-Base** | Page background, nav background |
| `#111214` / `#161618` | **Neutral-Surface** | Product UI panel backgrounds |
| `#1e2024` | **Neutral-Overlay** | Right metadata sidebar panel, slightly lighter surface |
| `#ffffff` | **Text-Main** | Hero headline, logo wordmark, "Sign up" button text |
| `#ffffff` with opacity ~0.85 | **Text-Inverse** (on dark) | Nav link labels (Product, Resources, etc.) |
| `#8a8f98` / `#6b7280` | **Text-Muted** | Subheadline body text ("Purpose-built for…"), sidebar nav labels, activity text |
| `#ffffff` | **Brand-Primary** | "Sign up" button fill, logo mark |
| `#2a2d35` / `#333640` | **Border-Default** | Panel dividers, sidebar separators, button outlines ("Log in" border if any) |
| `#5e6ad2` (estimated) | **Brand-Accent** | Likely used in status indicators / link highlights within app UI (blue-purple tones visible in status area) |
| `#f2994a` / amber | **Status-Warning** | "High" priority indicator (orange dot in right panel) |
| `#4caf7d` / green | **Status-Success** | "In Progress" status indicator (green circle) |
