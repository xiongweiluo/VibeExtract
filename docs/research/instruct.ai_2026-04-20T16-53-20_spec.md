...
> **Extracted:** Mon, 20 Apr 2026 16:53:20 GMT
> **URL:** https://instruct.ai/
> **Paradigm:** landing · **Density:** spacious · **Motif:** soft-topographic-texture, editorial-serif-headline, minimal-white-canvas, pill-tab-navigation, AI-prompt-centered
...
| Brand Secondary | #d1fae5 | `#d1fae5` |
| Brand Accent | #1d4ed8 | `#1d4ed8` |
| Background Page | #ffffff | `#ffffff` |
| Background Surface | #f5f5f4 | `#f5f5f4` |
...
|---|------|--------------|---------|------------|-----------------|
| 1 | **nav** | 0–8% | floating (fixed/sticky) | solid-color (white `#ffffff`) | interactive-controls |
| 2 | **hero** | 8–75% | base | image (faint topographic/world-map texture, very light green-grey wash over white) | typography + interactive-controls |
| 3 | **feature-grid** (partial) | 75–100% | base | solid-color (white `#ffffff`) | data/cards |
| 4 | **cookie-banner** (overlay) | ~78–100% lower-left corner | overlay | solid-color (white `#ffffff`) with border/shadow | interactive-controls + typography |
...
- **Column count:** 2 groups — left (logo + nav links) | right (CTA group)
- **Gap size:** ~24px between nav links (≈ spacing.lg), ~16px between right-side actions (≈ spacing.md)
- **Item pattern:** fluid
- **Z-index tier:** floating — position: fixed, z ≈ top-layer (above hero image)

### Hero Section — Filter Tab Bar
- **Layout engine:** Flexbox, horizontal, centered
- **Column count:** 4 tabs (Featured · Productivity · Creative · Lifestyle)
- **Gap size:** ~8px between tabs (≈ spacing.sm)
- **Item pattern:** fixed-width pill buttons
- **Active state:** "Featured" tab has filled background (white/opaque pill with border)

### Feature Grid (Cards — partial, below fold)
- **Layout engine:** CSS Grid — 2 columns (visible from screenshot: 2 side-by-side cards per row)
- **Column count:** 2
- **Gap size:** ~16–24px (≈ spacing.md–lg)
- **Item pattern:** fluid, equal-width
- **Masonry note:** Cards appear equal height per row — no masonry pattern detected

### Cookie Banner (overlay)
- **Layout engine:** Flexbox, vertical stack
- **Position type:** fixed, bottom-left
- **Z-index tier:** overlay — renders above all base content
- **Width:** ~305px fixed
...
| Asset | Status | Value/URL |
|-------|--------|-----------|
| **Hero background** | Detected | Faint topographic/world-map texture image — very low opacity, greenish-grey wash; specific URL **not present in Physical Data** → flagged as "no asset URL confirmed" |
| **Logo/brand** | Detected | Inline SVG icon (small square with "I" mark) + text "Instruct" — text-based brand with inline SVG mark |
| **Card images (feature grid)** | Partially visible | Four card thumbnails partially visible: (1) pink/coral gradient with search cursor UI, (2) green pixel/grid pattern, (3) Gmail+Meet icon composite, (4) Word+LinkedIn icon composite — **no confirmed URLs in Physical Data** → "no asset URL confirmed" |
| **Tab icons** | Detected | Inline SVGs within each tab pill (grid icon, trend arrow, palette/image icon, heart icon) — inline-svg set, count: 4 |
| **CTA chevron icon** | Detected | Inline SVG `>` arrow within "Get started for free" button |
| **Submit/mic icons** | Detected | Microphone SVG + return-arrow SVG in textarea action row — inline SVGs |

> **RULE applied:** No URLs invented. All card/hero image URLs marked "no asset URL confirmed" as Physical Data provides no matching asset list entries.
...
|-------------|--------------|----------|
| `#1a1a1a` / near-black | **Text-Main** | Hero headline "AI that works for you", nav links, card titles |
| `#ffffff` | **Neutral-Base / Neutral-Surface** | Nav bar background, page body, cookie banner background, textarea fill |
| `#1a1a1a` (filled) | **Brand-Primary** | "Get started" CTA button (top-right) — dark fill with white text |
| `#f5f5f4` / very light grey | **Neutral-Surface** | Textarea/input box background, active "Featured" pill tab |
| `#6b7280` / mid grey | **Text-Muted** | Subtitle text "Automate your work across apps, just by asking.", placeholder text in textarea, cookie banner body copy |
| `#e5e7eb` / light grey | **Border-Default** | Textarea border, pill tab borders, "Log in" button border |
| `#d1fae5` / very pale green-teal | **Brand-Secondary / Neutral-Overlay** | Hero background texture wash — extremely low opacity greenish topographic map |
| `#111827` / deep charcoal | **Brand-Primary (button)** | "Get started" button fill — close to black |
| `#ffffff` (on dark button) | **Text-Inverse** | White label text inside "Get started" dark button |
| `#374151` | **Text-Main (secondary)** | Cookie banner heading "Cookie Settings" |
| `#1d4ed8` / link blue | **Brand-Accent** | "Privacy Policy" hyperlink text in cookie banner |
| `#f9fafb` | **Neutral-Surface (subtle)** | "Reject" button fill in cookie banner (near-white pill) |
