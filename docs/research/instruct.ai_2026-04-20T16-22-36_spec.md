...
> **Extracted:** Mon, 20 Apr 2026 16:22:36 GMT
> **URL:** https://instruct.ai/
> **Paradigm:** saas-app · **Density:** comfortable · **Motif:** topographic-texture-background, serif-display-headline, ghost-pill-tabs, centered-input-delegate, monochromatic-near-white
...
| Brand Secondary | #374151 | `#374151` |
| Brand Accent | #2563eb | `#2563eb` |
| Background Page | #ffffff | `#ffffff` |
| Background Surface | #f3f4f6 | `#f3f4f6` |
| Background Overlay | #1a1a1a | #1a1a1a |
| Text Primary | #1a1c1a | `#1a1c1a` |
...
|---|------|--------------|---------|------------|-----------------|
| 1 | **nav** | 0–8% | floating (fixed) | solid-color (`#ffffff` / white) | interactive-controls + typography |
| 2 | **hero** | 8–75% | base | image (faint topographic/world-map pattern, very low opacity over near-white `#f0f5f3`) | typography + interactive-controls |
| 3 | **feature-grid** (partial) | 75–100% | base | solid-color (`#ffffff`) | imagery + typography |
| 4 | **cookie-banner** | ~78–100% (bottom-left) | overlay | solid-color (`#1a1a1a` dark) | interactive-controls + typography |
...
- **Gap size:** ~32px between nav links (xl), ~8–12px between right-side CTAs
- **Item pattern:** fluid
- **Z-layer:** z=100 (fixed/sticky), position=fixed

### Hero Section
- **Layout engine:** Flexbox (column, center-aligned)
- **Column count:** 1 (single centered column)
- **Gap size:** ~24px between elements (lg)
- **Item pattern:** fluid width, center-constrained (~640px max-width for input/buttons)
- **Category pill tabs:** Flexbox row, gap ≈ 8px (sm), wrapping

### Feature Grid (cards — partially visible)
- **Layout engine:** CSS Grid
- **Column count:** 2 (two equal-width columns visible)
- **Gap size:** ~24px gap (lg = 24px from scale)
- **Item pattern:** fluid, each card contains image thumbnail (right) + text (left)
- **Masonry note:** No masonry — uniform row heights

### Cookie Banner
- **Layout engine:** Flexbox (column)
- **Position:** fixed, bottom-left
- **Z-layer:** overlay, z > base content; position=fixed
- **Width:** ~300px fixed
- **Item pattern:** fixed-width block with text + two action buttons (row)
...
| Asset | Source | Value |
|-------|--------|-------|
| **Hero bg** | Physical Data / screenshot visual | Faint topographic/cartographic SVG or image texture overlay, very low opacity (~0.08) on `#eef4f0` near-white green-tinted background. No explicit URL confirmed in Physical Data assets. → **no asset URL found** |
| **Logo/brand** | Physical Data | Inline SVG — small square icon (dark, appears to be an abstract "I" or grid mark) + wordmark "Instruct" in dark text. → **inline-svg, text-only brand combination** |
| **Card images** | Physical Data assets | Up to 2 partial thumbnails visible: card 1 (Research & Summarize) shows a pink/coral gradient UI thumbnail; card 2 (Generate Podcast) shows a blurred/grainy video still with "▶ 0:13" overlay. → **no confirmed URLs in Physical Data** |
| **Category tab icons** | Screenshot | Inline SVG icons (grid icon, arrow-up-right icon, image/photo icon, heart icon) — **inline-svg set, count ≈ 4** |
| **Input area icons** | Screenshot | Microphone SVG + Enter/return key SVG button — **inline-svg, 2 icons** |
| **App integration icons** | Screenshot (Meeting prep card) | Small app logos (Word, LinkedIn icons visible) — **inline-svg or img, no URLs in Physical Data** |
...
|-------------|---------------|----------|
| `#1a1c1a` / `#111111` | **Text-Main** | Hero headline "AI that works for you", nav links, card titles |
| `#ffffff` | **Neutral-Surface** | Nav background, card backgrounds, input field background |
| `#eef4f1` | **Neutral-Base** | Hero section background (very slightly green-tinted near-white) |
| `#6b7280` | **Text-Muted** | Hero subheading "Automate your work…", input placeholder text, card description text |
| `#1a1a1a` | **Neutral-Overlay** | Cookie banner background (near-black) |
| `#ffffff` (on dark) | **Text-Inverse** | Cookie banner body text, "Accept" button label |
| `#111827` | **Brand-Primary** | "Get started" CTA button background (near-black/dark navy) |
| `#ffffff` | **Text-Inverse** | "Get started" button label text |
| `#e5e7eb` | **Border-Default** | "Log in" button border, input field border, category pill borders |
| `#f3f4f6` | **Neutral-Surface** | "Featured" active tab pill fill (very light grey) |
| `#374151` | **Text-Main** (secondary) | "Log in" button text, category pill labels |
| `#ffffff` (semi) | **Neutral-Surface** | Cookie "Reject" button background |
| `#2563eb` (approx) | **Brand-Accent** | "Privacy Policy" underlined link in cookie banner (blue link color) |
