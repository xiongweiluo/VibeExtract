...
> **Extracted:** Mon, 20 Apr 2026 17:13:20 GMT
> **URL:** https://www.pinterest.com/
> **Paradigm:** social-feed · **Density:** comfortable · **Motif:** bold-red-brand, purple-accent-headline, white-base, card-overlap-imagery, pill-buttons
...
This social feed is sustained by **a balanced composition of multiple forces**.
...
| Brand Secondary | #767676 | `#767676` |
| Brand Accent | #6d28d9 | `#6d28d9` |
| Background Page | #ffffff | `#ffffff` |
| Background Surface | #e9e9e9 | `#e9e9e9` |
| Background Overlay | rgba(0,0,0,0.08) | rgba(0,0,0,0.08) |
...
|---|------|--------------|---------|------------|-----------------|
| 1 | **nav** | 0–8% | floating (fixed) | solid-color (#fff) | interactive-controls |
| 2 | **hero** | 8–78% | base | solid-color (#fff) | typography + imagery |
| 3 | **cta-banner** | 78–100% | base | solid-color (#fff) | typography |

**Section Details:**

- **nav** — Fixed top bar containing Pinterest logo (SVG + wordmark), navigation links (Explore, Shop), centered search bar, secondary nav links (About, Businesses, Create, News), and two CTA buttons (Log in, Sign up). Height ≈ 64px.
- **hero** — Split-layout section: left side holds headline text ("Find ideas for / weeknight dinner"), carousel dot indicators, and two CTAs ("Join Pinterest for free" + "I already have an account"). Right side holds stacked food-image cards with a purple accent card. Includes a floating pause button (bottom-right).
- **cta-banner** — Partial section beginning at ~78% viewport height. Shows "Bring your favorite ideas to life" headline text, cut off by viewport bottom.
...
- **Column count:** 3 logical zones — [logo + primary nav] | [search bar] | [secondary nav + auth buttons]
- **Gap size:** ~16px (md) between nav links; ~8px (sm) between auth buttons
- **Item pattern:** fluid (search bar expands), fixed-width for buttons
- **Z-index tier:** floating, z ≈ 100, `position: fixed`, top: 0

### hero (split layout)
- **Layout engine:** Flexbox (row)
- **Column count:** 2 columns — left text column (~55% width), right image column (~45% width)
- **Gap size:** ~32px (xl) horizontal gap between columns
- **Item pattern:** fluid left, imagery right
- **Sub-layout (right image cards):**
  - Two overlapping image cards — main large card + smaller card offset lower-right
  - Absolute positioning with z-index layering for card overlap effect
  - Purple accent background (#7c3aed-family) on the foreground card container

### Carousel Dot Indicators
- **Layout engine:** Flexbox (row)
- **Item count:** 5 dots
- **Gap size:** ~8px (sm)
- **Item pattern:** fixed-width circles; active dot appears slightly larger/filled (~8px), inactive ~6px
- **Position:** below headline, above CTA buttons

### Floating Pause Button
- **Layout engine:** absolute/fixed
- **Position:** bottom-right of hero image area (~x:1159, y:641)
- **Z-index tier:** floating/overlay above hero image
- **Item pattern:** circular button ~40px diameter
...
| Asset | Source | Value |
|-------|--------|-------|
| **Hero bg** | none detected | plain white (#ffffff) background |
| **Logo/brand** | inline-svg + text | Pinterest red SVG logo mark + "Pinterest" wordmark |
| **Hero image 1 (main card)** | Physical Data / screenshot | Food photo — pomegranate-garnished meatballs dish on terracotta plate |
| **Hero image 2 (overlay card)** | Physical Data / screenshot | Food photo — hand holding a taco/wrap with orange/yellow filling |
| **Card background accent** | CSS background | Solid purple (#7c3aed-range) container behind overlay card |
| **Icon set** | inline-svg | Search icon (magnifying glass) in nav search field; Pause icon (‖) in hero pause button |
| **Card images** | Physical Data assets | ≤ 2 confirmed image assets visible; no additional card grid present |

> **Note:** No CDN image URLs were provided in Physical Data. Assets identified from screenshot visual evidence only. URLs marked as "no asset found" per rule.
...
|-------------|--------------|---------|
| `#e60023` | **Brand-Primary** | Pinterest logo SVG fill, "Join Pinterest for free" button background, "Log in" button background |
| `#7c3aed` (approx `#6d28d9`) | **Brand-Accent** | Hero headline second line "weeknight dinner" text color; purple card accent background in right image column |
| `#ffffff` | **Neutral-Base** | Page background (nav + hero), button text on colored buttons |
| `#111111` (near-black) | **Text-Main** | "Find ideas for" headline text (first line), nav links (Explore, Shop, About, etc.) |
| `#767676` (mid-gray) | **Text-Muted** | "I already have an account" link text; search bar placeholder text |
| `#e9e9e9` | **Neutral-Surface** | Search bar background fill; Sign up button background (light gray) |
| `#111111` | **Text-Inverse** | Sign up button text (dark on light gray) |
| `#cccccc` | **Border-Default** | Inactive carousel dots; subtle borders around search bar |
| `#767676` | **Brand-Secondary** | Secondary nav links (About, Businesses, Create, News) — muted weight |
| `#f4f4f4` | **Neutral-Overlay** | Pause button background (light circular button) |

---

### Discrepancy Notes

| Item | Visual Estimate | Physical Data | Resolution |
|------|----------------|--------------|------------|
| Body font size | ~14px visually | **12px** (Physical Data) | Defer to 12px |
| Headline size | ~56px visually | **60px** (Physical Data h1 step) | Use 60px |
| Sub-headline ("weeknight dinner") | ~40px visually | **36px** (Physical Data h3 step) | Use 36px |
| Font family | System sans visually | **Pin Sans** (Physical Data) | Use `font-family: 'Pin Sans'` |
| Body-lg weight | 500 visually implied | **500 confirmed** (Physical Data) | Use `font-weight: 500` |
