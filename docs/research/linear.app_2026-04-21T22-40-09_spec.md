...
> **Extracted:** Tue, 21 Apr 2026 22:40:09 GMT
> **URL:** https://linear.app/
> **Paradigm:** saas-app · **Density:** comfortable · **Motif:** dark-obsidian surfaces, razor-thin hairline borders, monochromatic editorial typography, app-UI-as-hero showcase, minimal chromatic contrast
...
This SaaS application is sustained by **large-format photography and imagery**.

### Evidence
- 23 image assets detected (heavy image density)
- Hero section: full-bleed layout — "The productdevelopmentsystem for teamsand agentsThe product …"
...
| Brand Secondary | #5e6ad2 | `#5e6ad2` |
| Brand Accent | #f2994a | `#f2994a` |
| Background Page | #08080c | `#08080c` |
| Background Surface | #23252a | `#23252a` |
| Background Overlay | #2d2f35 | #2d2f35 |
| Text Primary | #ffffff | `#ffffff` |
| Text Secondary | #9b9ba8 | `#9b9ba8` |
| Text Inverse | #000000 | `#000000` |
| Border | #23252a | `#23252a` |
| Status Success | #10B981 | `#10B981` |
| Status Warning | #f2994a | `#f2994a` |
...
| Component | Details |
|---|---|
| Hero | present · full-bleed · "The productdevelopmentsystem for teamsand agentsThe product " · 0 CTAs |
...
- https://linear.app/cdn-cgi/imagedelivery/fO02fVwohEs9s9UHFwon6A/5a228df7-2423-4807-6105-cd32ddddde00/f=auto,dpr=2,q=95,fit=scale-down,metadata=none
_…and 3 more_
...
<details><summary>SVG #2 (448 chars)</summary>
...
<details><summary>SVG #4 (348 chars)</summary>

```html
<svg class="" width="16" height="16" viewBox="0 0 16 16" fill="var(--label-faint)" role="img" focusable="false" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" style="--icon-color: var(--label-faint);"><path d="M4.53 5.47a.75.75 0 0 0-1.06 1.06l4 4a.75.75 0 0 0 1.054.007l4-3.903a.75.75 0 0 0-1.048-1.073l-3.47 3.385L4.53 5.47Z"></path></svg>
```
...
|---|------|--------------|---------|------------|-----------------|
| 1 | **nav** | 0–8% | floating (fixed/sticky) | solid-color `#000000` / near-black | interactive-controls |
| 2 | **hero** | 8–60% | base | solid-color `#000000` / very dark `#0a0a0d` | typography |
| 3 | **product-preview** | 60–100% | base | solid-color dark (continuation of hero bg) | imagery (app UI screenshot/panel) |

**Detailed Notes:**

- **nav**: Full-width bar containing Linear logo (icon + wordmark) left, center navigation links (Product, Resources, Customers, Pricing, Now, Contact), a vertical divider, then Log In (text) + Sign Up (outlined button) right-aligned.
- **hero**: Large headline block ("The product development system for teams and agents"), two-line sub-tagline ("Purpose-built for planning and building products. / Designed for the AI era."). Significant vertical whitespace above and below headline.
- **product-preview**: Rendered application UI mockup (Linear app interface) showing sidebar nav, issue detail panel, and right-side metadata panel — begins ~60% down the viewport and extends below the fold.
...
- **Column count**: 3 logical zones (logo | nav-links | auth-ctas)
- **Gap size**: Links spaced approximately 24px apart (spacing.lg)
- **Item pattern**: fluid width, centered
- **Z-index tier**: Floating — `position: fixed` or `position: sticky`, z ≈ top layer (likely z=100+)
- **Shadow**: `rgba(0,0,0,0.4) 0px 1px 0px 0px` bottom edge separator

### Hero Text Block
- **Layout engine**: Flexbox / block flow (single column)
- **Column count**: 1
- **Gap size**: Headline-to-tagline gap ≈ 32px (spacing.xl); above-headline breathing room ≈ 120–160px from nav
- **Item pattern**: Full-width text, left-aligned
- **Typography (headline)**: Physical Data confirms **64px / 4rem, lh:1.0, w:510, ls:−1.408px** — "The product development system for teams and agents"
- **Typography (tagline)**: Matches **15px / 0.9375rem, lh:1.6, w:400, ls:−0.165px**

### Product Preview (App Mockup)
- **Layout engine**: Absolute / CSS Grid (3-column panel layout within the mockup)
- **Column count**: 3 distinct panes visible — sidebar (~220px) | main content | right metadata panel
- **Gap size**: Panel dividers via `box-shadow: rgb(35,37,42) 0px 0px 0px 1px inset` (1px inset borders, no true gap)
- **Item pattern**: Fixed-width sidebar; fluid middle; fixed-width right panel
- **Card/row shadow**: `rgba(0,0,0,0.03) 0px 1.2px 0px 0px` — [×16] indicates list row separators throughout the app UI
- **Masonry note**: No masonry pattern — strict horizontal panel split

---

## ASSET AUDIT

| Asset Role | Value / Source |
|------------|----------------|
| **Hero bg** | Solid dark color — `#000000` or near-black `#0a0a0d`; no image/gradient detected in Physical Data |
| **Logo/brand** | Inline SVG icon (geometric hexagonal/diamond mark) + "Linear" wordmark text — visible top-left in nav; referenced as inline-svg. Text rendered in Inter Variable |
| **Card images** | No card images in this viewport section. App mockup panes are rendered HTML/CSS simulation, not `<img>` assets |
| **Icon set** | Inline SVGs — small status icons visible within app mockup (In Progress circle, High priority, avatar circles). Count: ~6–8 small inline SVGs within the product preview pane |
| **Hero image** | no asset found — hero is text-only against solid background |
| **Background texture/gradient** | no asset found — Physical Data shows no gradient or image URL |
...
|---|---|---|
| `#000000` / `#08080c` | **Neutral-Base** | Full page background (nav + hero + preview container) |
| `#ffffff` | **Text-Inverse / Text-Main** | Hero headline "The product development system…" white on black |
| `#9b9ba8` | **Text-Muted** | Tagline body text ("Purpose-built for planning…"), nav link labels |
| `#ffffff` | **Brand-Primary (text + button label)** | "Sign up" button label; logo wordmark "Linear" |
| `#23252a` | **Neutral-Surface** | App mockup panel backgrounds (sidebar, right panel); matches Physical Data `rgb(35,37,42)` from inset shadow |
| `#2d2f35` | **Neutral-Overlay** | App mockup content pane background (slightly lighter than sidebar) |
| `rgba(35,37,42,1)` | **Border-Default** | Panel borders / row separators via `rgb(35,37,42) 0px 0px 0px 1px inset` (Physical Data ×5) |
| `rgba(0,0,0,0.03)` | **Border-Default (subtle)** | List row bottom hairlines `rgba(0,0,0,0.03) 0px 1.2px 0px 0px` (Physical Data ×16) |
| `#ffffff` with 1px border | **Brand-Accent (CTA border)** | "Sign up" button — outlined style, white border, dark bg, white text |
| `#5e6ad2` *(visual estimate)* | **Brand-Secondary / Status-Info** | "In Progress" status indicator circle in right metadata panel (blue-purple tint) |
| `#f2994a` *(visual estimate)* | **Status-Warning** | "High" priority indicator (orange dot) in right metadata panel |

**Key Discrepancy Note:** Physical Data provides no explicit hex color variables beyond what's embedded in box-shadow RGB values (`rgb(35,37,42)` confirmed). All other hex values above are derived from screenshot visual analysis and should be validated against CSS custom properties or computed color values from the live DOM.
