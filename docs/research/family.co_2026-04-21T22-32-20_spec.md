...
> **Extracted:** Tue, 21 Apr 2026 22:32:20 GMT
> **URL:** https://family.co/
> **Paradigm:** landing · **Density:** comfortable · **Motif:** warm off-white base, scattered playful emoji-style illustrations, bold centered typography, dark pill CTAs, crypto-meets-consumer-friendly
...
**Families:** body — `Inter, sans-serif` · heading — `Inter, sans-serif` · mono — `ui-monospace, monospace`
...
|---|------|--------------|---------|------------|-----------------|
| 1 | **nav** | 0–8% | floating (sticky/fixed) | solid-color (#FFFFFF or near-white) | interactive-controls |
| 2 | **hero** | 8–62% | base | solid-color (off-white/warm white) | typography + imagery |
| 3 | **feature-intro** | 62–75% | base | solid-color (off-white/warm white) | typography |
| 4 | **feature-grid** (partial) | 75–100% | base | solid-color (off-white/warm white) | imagery + data |
...
- **Column count**: N/A — horizontal row with 3 zones: logo-left, nav-links-center, cta-right
- **Gap size**: approx. 24px (lg) between nav items
- **Item pattern**: fluid / space-between
- **Z-index tier**: floating — positioned above hero content; fixed/sticky
- Nav dropdowns: "Developers ▾", "Resources ▾", "X (Twitter)" in center cluster; "Log In" + "Get Started" on right

### Hero (base)
- **Layout engine**: Flexbox (centered column)
- **Column count**: 1 (text stack centered horizontally)
- **Gap size**: ~20px between heading, subtext, and CTA buttons (spacing scale md=16px to lg=24px)
- **Item pattern**: fluid, center-aligned
- **CTA buttons**: side-by-side Flexbox row — "Download on iOS" (filled/dark) + "Watch the Video" (lighter/outline-adjacent), gap ≈ 16px (md)
- **Decorative illustrations**: absolutely positioned left and right of center text — scattered emoji-style characters (blob, icons, stars) — not in normal flow
- **Masonry note**: none

### Feature-Grid (partial, bottom of viewport)
- **Layout engine**: CSS Grid (inferred from 3-column card pattern visible at bottom)
- **Column count**: 3 (three card columns visible — "Send", center card with green icon, "Submitted" card)
- **Gap size**: ≈ 24px (lg) between columns
- **Item pattern**: fluid / equal-width columns
- **Masonry note**: none visible yet

---

## ASSET AUDIT

- **Hero bg**: No distinct image URL — background is a solid warm off-white color (`color(display-p3 0.94902 0.941176 0.929412)` as referenced in the box-shadow inset — this same warm tone is the page base color: approximately **#F2EFED**)
- **Logo/brand**: Inline SVG or icon glyph (grid/box symbol) + wordmark "Family" — rendered as text + SVG icon combination; no external image URL found in Physical Data
- **Hero illustrations**: Scattered decorative characters (blue blob with eyes, smiling green blob, cat character, emoji icons — stars, hearts, coins, lock, checkmark, arrows, Ethereum diamond) — these appear to be inline SVGs or a single sprite image; no specific asset URL confirmed in Physical Data → **no asset URL found**
- **Card images**: Bottom partial cards show:
  - "Send" card: Telegram-style paper-plane icon (blue circle) — no URL confirmed
  - Center card: Green rounded icon — no URL confirmed
  - "Submitted" card: Timeline/status UI — no URL confirmed
  - All marked **no asset found** per Physical Data
- **Icon set**: Inline SVGs inferred (Apple logo in CTA, play triangle in "Watch the Video"); count not confirmed in Physical Data → inline SVG, count unverified
...
|---|---|---|
| `#F2EFED` (warm off-white, ~`color(display-p3 0.949 0.941 0.929)`) | **Neutral-Base** | Full page background, hero section, nav background |
| `#1A1A1A` / near-black | **Text-Main** | Hero H1 "Your favorite crypto wallet.", feature heading, nav links |
| `#4A4A4A` / dark gray | **Text-Muted** | Hero subtitle "Explore Ethereum with the best wallet for iOS…" |
| `#1A1A1A` / near-black | **Brand-Primary** | "Download on iOS" button fill (dark/black pill button) |
| `#FFFFFF` | **Text-Inverse** | Text inside dark "Download on iOS" button |
| `#FFFFFF` or very light | **Neutral-Surface** | "Watch the Video" button background (lighter pill, ghost/outline style) |
| `#1A1A1A` | **Text-Main** | "Watch the Video" button text, nav items |
| `#FFFFFF` | **Neutral-Surface** | Nav bar background (appears white/near-white against off-white page) |
| `#1A1A1A` (border-radius pill, dark bg) | **Brand-Primary** | "Get Started" nav CTA button — matches "Download on iOS" treatment |
| `#F2EFED` approx | **Neutral-Overlay** | "Log In" button — appears as outlined/ghost against same warm background |

**Key Color Note**: The Physical Data box-shadow value `color(display-p3 0.94902 0.941176 0.929412)` confirms the warm off-white base is `≈ #F2EFED` — this is a recurring inset shadow on buttons/cards, suggesting buttons and panels share this warm neutral as their surface or highlight inset tone.
